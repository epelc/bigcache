...
BigCache keeps entries on heap but omits GC for them. To achieve that operations on bytes slices take place,
...
Entries are kept in bytes slices, to omit GC again.
Bytes slices size can grow to gigabytes without impact on performance
